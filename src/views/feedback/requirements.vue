...
<!--工具条-->
    ...
    <!--工具条-->
      ...
      <el-pagination v-show="total>0" current-page="1" :page-sizes="[10,20,30, 50]" page-size="2" :total="10" background layout="total, sizes, prev, pager, next, jumper"  @current-change="handleCurrentChange"/>
    </div>
		</el-col>
  </section>
...
<script>
import { getRequirements, createRequirements, editRequirements, deleteRequirements } from '../../api/api'
export default {
  data() {
    return {
      total: 10,
  ...
  methods: {
    ...
    addSubmit() {
      ...
      createRequirements(this.addForm).then(response => { this.requirements.push(response.data.data) })
    },
    editSubmit() {
